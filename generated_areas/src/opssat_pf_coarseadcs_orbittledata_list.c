...
struct _opssat_pf_coarseadcs_orbittledata_list_t {
  unsigned int element_count;
  opssat_pf_coarseadcs_orbittledata_t ** content;
};

// default constructor
opssat_pf_coarseadcs_orbittledata_list_t * opssat_pf_coarseadcs_orbittledata_list_new(unsigned int element_count)
{
  opssat_pf_coarseadcs_orbittledata_list_t *self = (opssat_pf_coarseadcs_orbittledata_list_t *) calloc(1, sizeof(opssat_pf_coarseadcs_orbittledata_list_t));
  if (!self)
    return NULL;
  self->element_count = element_count;
  self->content = (opssat_pf_coarseadcs_orbittledata_t **) calloc(element_count, sizeof(opssat_pf_coarseadcs_orbittledata_t *));
  ...
  if (self_p && *self_p)
  {
    for (int i = 0; i < (*self_p)->element_count; i++)
    {
      if ((*self_p)->content[i] != NULL)
        opssat_pf_coarseadcs_orbittledata_destroy(&(*self_p)->content[i]);
    }
    free((*self_p)->content);
  }
  free (*self_p);
...
int opssat_pf_coarseadcs_orbittledata_list_add_encoding_length_malbinary(opssat_pf_coarseadcs_orbittledata_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  {
    opssat_pf_coarseadcs_orbittledata_t * list_element = self->content[i];
    ...
    if (presence_flag)
    {
      rc = opssat_pf_coarseadcs_orbittledata_add_encoding_length_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int opssat_pf_coarseadcs_orbittledata_list_encode_malbinary(opssat_pf_coarseadcs_orbittledata_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  if (rc < 0)
    return rc;
  opssat_pf_coarseadcs_orbittledata_t ** content = self->content;
  for (int i = 0; i < list_size; i++)
  {
    opssat_pf_coarseadcs_orbittledata_t *list_element = content[i];
    bool presence_flag = (list_element != NULL);
    ...
    if (presence_flag)
    {
      rc = opssat_pf_coarseadcs_orbittledata_encode_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int opssat_pf_coarseadcs_orbittledata_list_decode_malbinary(opssat_pf_coarseadcs_orbittledata_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  self->content = (opssat_pf_coarseadcs_orbittledata_t **) calloc(list_size, sizeof(opssat_pf_coarseadcs_orbittledata_t *));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      self->content[i] = opssat_pf_coarseadcs_orbittledata_new();
      if (self->content[i] == NULL) return -1;
      rc = opssat_pf_coarseadcs_orbittledata_decode_malbinary(self->content[i], decoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
{
  printf(" * list of opssat_pf_coarseadcs_orbittledata: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
