...
struct _opssat_pf_coarseadcs_adcsmode_list_t {
  unsigned int element_count;
  bool * presence_flags;
  opssat_pf_coarseadcs_adcsmode_t * content;
};

// default constructor
opssat_pf_coarseadcs_adcsmode_list_t * opssat_pf_coarseadcs_adcsmode_list_new(unsigned int element_count)
{
  opssat_pf_coarseadcs_adcsmode_list_t *self = (opssat_pf_coarseadcs_adcsmode_list_t *) calloc(1, sizeof(opssat_pf_coarseadcs_adcsmode_list_t));
  if (!self)
    return NULL;
  ...
  self->content = (opssat_pf_coarseadcs_adcsmode_t *) calloc(element_count, sizeof(opssat_pf_coarseadcs_adcsmode_t));
  if (!self->content)
  {
...
int opssat_pf_coarseadcs_adcsmode_list_decode_malbinary(opssat_pf_coarseadcs_adcsmode_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  if (self->presence_flags == NULL)
    return -1;
  self->content = (opssat_pf_coarseadcs_adcsmode_t *) calloc(list_size, sizeof(opssat_pf_coarseadcs_adcsmode_t));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      ...
      if (rc < 0)
        return rc;
      self->content[i] = (opssat_pf_coarseadcs_adcsmode_t) enumerated_value;
    }
  }
  return rc;
}
...
{
  printf(" * list of opssat_pf_coarseadcs_adcsmode: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
