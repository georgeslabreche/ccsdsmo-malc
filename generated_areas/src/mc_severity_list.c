...
struct _mc_severity_list_t {
  unsigned int element_count;
  bool * presence_flags;
  mc_severity_t * content;
};

// default constructor
mc_severity_list_t * mc_severity_list_new(unsigned int element_count)
{
  mc_severity_list_t *self = (mc_severity_list_t *) calloc(1, sizeof(mc_severity_list_t));
  if (!self)
    return NULL;
  ...
  self->content = (mc_severity_t *) calloc(element_count, sizeof(mc_severity_t));
  if (!self->content)
  {
...
int mc_severity_list_decode_malbinary(mc_severity_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  if (self->presence_flags == NULL)
    return -1;
  self->content = (mc_severity_t *) calloc(list_size, sizeof(mc_severity_t));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      ...
      if (rc < 0)
        return rc;
      self->content[i] = (mc_severity_t) enumerated_value;
    }
  }
  return rc;
}
