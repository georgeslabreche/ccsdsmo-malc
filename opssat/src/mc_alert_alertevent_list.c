...
struct _mc_alert_alertevent_list_t {
  unsigned int element_count;
  mc_alert_alertevent_t ** content;
};

// default constructor
mc_alert_alertevent_list_t * mc_alert_alertevent_list_new(unsigned int element_count)
{
  mc_alert_alertevent_list_t *self = (mc_alert_alertevent_list_t *) calloc(1, sizeof(mc_alert_alertevent_list_t));
  if (!self)
    return NULL;
  self->element_count = element_count;
  self->content = (mc_alert_alertevent_t **) calloc(element_count, sizeof(mc_alert_alertevent_t *));
  ...
  if (self_p && *self_p)
  {
    for (int i = 0; i < (*self_p)->element_count; i++)
    {
      if ((*self_p)->content[i] != NULL)
        mc_alert_alertevent_destroy(&(*self_p)->content[i]);
    }
    free((*self_p)->content);
  }
  free (*self_p);
...
int mc_alert_alertevent_list_add_encoding_length_malbinary(mc_alert_alertevent_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  {
    mc_alert_alertevent_t * list_element = self->content[i];
    ...
    if (presence_flag)
    {
      rc = mc_alert_alertevent_add_encoding_length_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int mc_alert_alertevent_list_encode_malbinary(mc_alert_alertevent_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  if (rc < 0)
    return rc;
  mc_alert_alertevent_t ** content = self->content;
  for (int i = 0; i < list_size; i++)
  {
    mc_alert_alertevent_t *list_element = content[i];
    bool presence_flag = (list_element != NULL);
    ...
    if (presence_flag)
    {
      rc = mc_alert_alertevent_encode_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int mc_alert_alertevent_list_decode_malbinary(mc_alert_alertevent_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  self->content = (mc_alert_alertevent_t **) calloc(list_size, sizeof(mc_alert_alertevent_t *));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      self->content[i] = mc_alert_alertevent_new();
      if (self->content[i] == NULL) return -1;
      rc = mc_alert_alertevent_decode_malbinary(self->content[i], decoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
{
  printf(" * list of mc_alert_alertevent: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
