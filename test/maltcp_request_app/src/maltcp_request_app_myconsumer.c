...
// state
struct _maltcp_request_app_myconsumer_t {
...
maltcp_request_app_myconsumer_t *maltcp_request_app_myconsumer_new(mal_uri_t *provider_uri,
    ...
    void *decoder) {
  maltcp_request_app_myconsumer_t *self = (maltcp_request_app_myconsumer_t *) malloc(
      sizeof(maltcp_request_app_myconsumer_t));
  if (!self)
    return NULL;
...
mal_identifier_t *maltcp_request_app_myconsumer_get_network_zone(
    maltcp_request_app_myconsumer_t *self) {
  return self->network_zone;
}

mal_uinteger_t maltcp_request_app_myconsumer_get_priority(maltcp_request_app_myconsumer_t *self) {
  return self->priority;
}

mal_qoslevel_t maltcp_request_app_myconsumer_get_qoslevel(maltcp_request_app_myconsumer_t *self) {
  return self->qoslevel;
}

mal_sessiontype_t maltcp_request_app_myconsumer_get_session(
    maltcp_request_app_myconsumer_t *self) {
  return self->session;
}

mal_identifier_t *maltcp_request_app_myconsumer_get_session_name(
    maltcp_request_app_myconsumer_t *self) {
  return self->session_name;
}

int maltcp_request_app_myconsumer_initialize(void *self, mal_actor_t *mal_actor) {
  printf("maltcp_request_app_myconsumer_initialize()\n");
  ...
  rc = mal_routing_register_consumer_request_handler(
      ...
      maltcp_request_app_myconsumer_testarea_testservice_testrequest_response);

  maltcp_request_app_myconsumer_t *consumer = (maltcp_request_app_myconsumer_t *) self;

  // initiate a request interaction
  mal_uri_t *provider_uri = consumer->provider_uri;

  printf("maltcp_request_app_myconsumer: provider URI: %s\n", provider_uri);

  printf("maltcp_request_app_myconsumer: new test composite\n");
  ...
  printf("maltcp_request_app_myconsumer: new string list\n");
  ...
  printf("maltcp_request_app_myconsumer: encoding_length_0\n");
  ...
  printf("maltcp_request_app_myconsumer: encoding_length_1\n");
  ...
  printf("maltcp_request_app_myconsumer: new MAL message\n");
  ...
  printf("maltcp_request_app_myconsumer: encode 0\n");
  rc = testarea_testservice_testrequest_request_encode_0(
      consumer->encoding_format_code, bytes, &offset,
      maltcp_request_app_myconsumer_get_encoder(consumer), testcomposite);
  if (rc < 0)
    return rc;

  printf("maltcp_request_app_myconsumer: encode 1\n");
  rc = testarea_testservice_testrequest_request_encode_1(
      consumer->encoding_format_code, bytes, &offset,
      maltcp_request_app_myconsumer_get_encoder(consumer), string_list);
  if (rc < 0)
    return rc;

  printf("maltcp_request_app_myconsumer: request message\n");
...
int maltcp_request_app_myconsumer_testarea_testservice_testrequest_response(
  ...
  maltcp_request_app_myconsumer_t *consumer = (maltcp_request_app_myconsumer_t *) self;
	...
	printf("#### maltcp_request_app_myconsumer, testrequest: %d, %d\n",
...
void maltcp_request_app_myconsumer_test(bool verbose) {
  printf(" * maltcp_request_app_myconsumer_test: ");
  if (verbose)
    printf("\n");
