...
//  --------------------------------------------------------------------------
//  Selftest
int maltcp_submit_app_create_provider(
    bool verbose,
    mal_ctx_t *mal_ctx,
  ...
  printf(" * maltcp_submit_app_create_provider: \n");

  maltcp_submit_app_myprovider_t *provider = maltcp_submit_app_myprovider_new(MALBINARY_FORMAT_CODE, encoder, decoder);

//  mal_actor_t *provider_actor =
  mal_actor_new(
      mal_ctx,
      provider_uri, provider,
      maltcp_submit_app_myprovider_initialize, maltcp_submit_app_myprovider_finalize);

  return rc;
}

int maltcp_submit_app_create_consumer(
    bool verbose,
    mal_ctx_t *mal_ctx,
    mal_uri_t *provider_uri,
    malbinary_encoder_t *encoder,
  ...
  printf(" * maltcp_submit_app_create_consumer: \n");
  ...
  maltcp_submit_app_myconsumer_t *consumer = maltcp_submit_app_myconsumer_new(provider_uri,
      authentication_id, qoslevel, priority, domain, network_zone, session,
      session_name, MALBINARY_FORMAT_CODE, encoder, decoder);

  mal_uri_t *consumer_uri = mal_ctx_create_uri(mal_ctx, "maltcp_submit_app/myconsumer");
  printf("maltcp_submit_app: consumer URI: %s\n", consumer_uri);

//  mal_actor_t *consumer_actor =
  mal_actor_new(
      mal_ctx,
      consumer_uri, consumer,
      maltcp_submit_app_myconsumer_initialize, maltcp_submit_app_myconsumer_finalize);

  return rc;
}

void maltcp_submit_app_test(bool verbose) {
  printf(" * maltcp_submit_app: \n");
  ...
  mal_uri_t *provider_uri = mal_ctx_create_uri(mal_ctx, "maltcp_submit_app/myprovider");
  printf("maltcp_submit_app: provider URI: %s\n", provider_uri);

  maltcp_submit_app_create_provider(verbose, mal_ctx, provider_uri, encoder, decoder);
  maltcp_submit_app_create_consumer(verbose, mal_ctx, provider_uri, encoder, decoder);
