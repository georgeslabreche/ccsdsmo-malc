...
struct _mal_finetime_list_t {
  unsigned int element_count;
  bool *presence_flags;
  mal_finetime_t *content;
};

mal_finetime_list_t *mal_finetime_list_new(unsigned int element_count) {
  mal_finetime_list_t *self = (mal_finetime_list_t *) malloc(
      sizeof(mal_finetime_list_t));
  if (!self)
    return NULL;
  ...
  self->content = (mal_finetime_t *) malloc(
      sizeof(mal_finetime_t) * element_count);
...
void mal_finetime_list_destroy(mal_finetime_list_t **self_p) {
  if (*self_p) {
    mal_finetime_list_t *self = *self_p;
...
int mal_finetime_list_add_encoding_length_malbinary(mal_finetime_list_t *self,
  ...
  mal_finetime_t *content = self->content;
  // Presence flags
  malbinary_add_length((malbinary_cursor_t *) cursor, list_size);
  for (int i = 0; i < list_size; i++) {
    if (presence_flags[i]) {
      mal_finetime_t list_element = content[i];
      rc = malbinary_encoder_add_finetime_encoding_length(encoder, list_element, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int mal_finetime_list_encode_malbinary(mal_finetime_list_t *self,
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (present) {
      mal_finetime_t list_element = content[i];
      rc = malbinary_encoder_encode_finetime(encoder, cursor,
          list_element);
      if (rc < 0)
        return rc;
    }
...
int mal_finetime_list_decode_malbinary(mal_finetime_list_t *self,
  ...
  mal_finetime_t *list_content = (mal_finetime_t *) malloc(
      sizeof(mal_finetime_t) * list_size);
  for (int i = 0; i < list_size; i++) {
    mal_finetime_t list_element;
    bool presence_flag;
    ...
    if (presence_flag) {
      rc = malbinary_decoder_decode_finetime(decoder, cursor,
          &list_element);
      if (rc < 0)
        return rc;
