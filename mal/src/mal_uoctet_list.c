...
struct _mal_uoctet_list_t {
  unsigned int element_count;
  bool *presence_flags;
  mal_uoctet_t *content;
};

mal_uoctet_list_t *mal_uoctet_list_new(unsigned int element_count) {
  mal_uoctet_list_t *self = (mal_uoctet_list_t *) malloc(
      sizeof(mal_uoctet_list_t));
  if (!self)
    return NULL;
  ...
  self->content = (mal_uoctet_t *) malloc(
      sizeof(mal_uoctet_t) * element_count);
...
int mal_uoctet_list_add_encoding_length_malbinary(mal_uoctet_list_t *self,
  ...
  mal_uoctet_t *content = self->content;
  // Presence flags
  malbinary_add_length((malbinary_cursor_t *) cursor, list_size);
  for (int i = 0; i < list_size; i++) {
    if (presence_flags[i]) {
      mal_uoctet_t list_element = content[i];
      rc = malbinary_encoder_add_uoctet_encoding_length(encoder, list_element, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int mal_uoctet_list_encode_malbinary(mal_uoctet_list_t *self,
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (present) {
      mal_uoctet_t list_element = content[i];
      rc = malbinary_encoder_encode_uoctet(encoder, cursor,
          list_element);
      if (rc < 0)
        return rc;
    }
...
int mal_uoctet_list_decode_malbinary(mal_uoctet_list_t *self,
  ...
  mal_uoctet_t *list_content = (mal_uoctet_t *) malloc(
      sizeof(mal_uoctet_t) * list_size);
  for (int i = 0; i < list_size; i++) {
    mal_uoctet_t list_element;
    bool presence_flag;
    ...
    if (presence_flag) {
      rc = malbinary_decoder_decode_uoctet(decoder, cursor,
          &list_element);
      if (rc < 0)
        return rc;
