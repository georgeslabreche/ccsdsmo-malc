...
mal_identifier_list_t *mal_identifier_list_new(unsigned int element_count) {
  mal_identifier_list_t *self = (mal_identifier_list_t *) calloc(1, sizeof(mal_identifier_list_t));
  if (!self)
    return NULL;
...
int mal_identifier_list_add_encoding_length_malbinary(mal_identifier_list_t *self,
  ...
  {
    mal_identifier_t *list_element = self->content[i];
    bool presence_flag = (list_element != NULL);
    mal_encoder_add_presence_flag_encoding_length(encoder, cursor, presence_flag);
    if (presence_flag)
    {
      rc = mal_encoder_add_identifier_encoding_length(encoder, list_element, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int mal_identifier_list_encode_malbinary(mal_identifier_list_t *self,
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      rc = mal_encoder_encode_identifier(encoder, cursor, list_element);
      if (rc < 0)
        return rc;
    }
  }
...
int mal_identifier_list_decode_malbinary(mal_identifier_list_t *self,
  ...
  self->content = (mal_identifier_t **) calloc(self->element_count, sizeof(mal_identifier_t *));
  ...
  for (int i = 0; i < self->element_count; i++)
  {
    ...
    if (presence_flag)
    {
      rc = mal_decoder_decode_identifier(decoder, cursor, &self->content[i]);
