...
struct _mal_qoslevel_list_t {
  unsigned int element_count;
  bool * presence_flags;
  mal_qoslevel_t * content;
};

// default constructor
mal_qoslevel_list_t * mal_qoslevel_list_new(unsigned int element_count)
{
  mal_qoslevel_list_t *self = (mal_qoslevel_list_t *) calloc(1, sizeof(mal_qoslevel_list_t));
  if (!self)
    return NULL;
  ...
  self->content = (mal_qoslevel_t *) calloc(element_count, sizeof(mal_qoslevel_t));
  if (!self->content)
  {
...
int mal_qoslevel_list_add_encoding_length_malbinary(mal_qoslevel_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  int rc = 0;
...
int mal_qoslevel_list_encode_malbinary(mal_qoslevel_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  int rc = 0;
  unsigned int list_size = self->element_count;
...
int mal_qoslevel_list_decode_malbinary(mal_qoslevel_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  int rc = 0;
  ...
  if (self->presence_flags == NULL)
    return -1;
  self->content = (mal_qoslevel_t *) calloc(list_size, sizeof(mal_qoslevel_t));
