...
mal_string_list_t *mal_string_list_new(unsigned int element_count) {
  mal_string_list_t *self = (mal_string_list_t *) malloc(
      sizeof(mal_string_list_t));
  if (!self)
    return NULL;

  self->element_count = element_count;
  self->content = (mal_string_t **) malloc(
      sizeof(mal_string_t *) * element_count);
...
void mal_string_list_destroy(mal_string_list_t **self_p) {
  if (*self_p) {
    mal_string_list_t *self = *self_p;
    unsigned int list_size = self->element_count;
    mal_string_t **content = self->content;
    for (int i = 0; i < list_size; i++) {
      mal_string_destroy(&content[i]);
    }
    free(self->content);
    free(self);
    *self_p = NULL;
