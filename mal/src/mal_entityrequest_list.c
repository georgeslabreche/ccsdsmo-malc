...
{
  mal_entityrequest_list_t *self = (mal_entityrequest_list_t *) calloc(1, sizeof(mal_entityrequest_list_t));
  if (!self)
    return NULL;
  self->element_count = element_count;
  self->content = (mal_entityrequest_t **) calloc(element_count, sizeof(mal_entityrequest_t *));
...
int mal_entityrequest_list_add_encoding_length_malbinary(mal_entityrequest_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  int rc = 0;
  ...
  {
    mal_entityrequest_t * list_element = self->content[i];
...
int mal_entityrequest_list_encode_malbinary(mal_entityrequest_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  int rc = 0;
  unsigned int list_size = self->element_count;
...
int mal_entityrequest_list_decode_malbinary(mal_entityrequest_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  int rc = 0;
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      self->content[i] = mal_entityrequest_new();
      if (self->content[i] == NULL) return -1;
      rc = mal_entityrequest_decode_malbinary(self->content[i], decoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
{
  printf(" * list of mal_entityrequest: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
