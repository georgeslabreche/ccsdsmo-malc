...
int mal_entityrequest_list_add_encoding_length_malbinary(mal_entityrequest_list_t * self, mal_encoder_t * mal_encoder, void * cursor)
{
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      rc = mal_entityrequest_add_encoding_length_malbinary(list_element, mal_encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int mal_entityrequest_list_encode_malbinary(mal_entityrequest_list_t * self, mal_encoder_t * mal_encoder, void * cursor)
{
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      rc = mal_entityrequest_encode_malbinary(list_element, mal_encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int mal_entityrequest_list_decode_malbinary(mal_entityrequest_list_t * self, mal_decoder_t * mal_decoder, void * cursor)
{
  ...
  self->content = (mal_entityrequest_t **) calloc(self->element_count, sizeof(mal_entityrequest_t *));
  ...
  for (int i = 0; i < self->element_count; i++)
  {
    ...
    if (presence_flag)
    {
      ...
      rc = mal_entityrequest_decode_malbinary(self->content[i], mal_decoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
