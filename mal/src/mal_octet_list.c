...
mal_octet_list_t *mal_octet_list_new(unsigned int element_count) {
  mal_octet_list_t *self = (mal_octet_list_t *) calloc(1, sizeof(mal_octet_list_t));
  if (!self)
    return NULL;

  self->element_count = element_count;
  self->presence_flags = (bool *) calloc(element_count, sizeof(bool));
  self->content = (mal_octet_t *) calloc(element_count, sizeof(mal_octet_t));
...
int mal_octet_list_encode_malbinary(mal_octet_list_t *self,
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      rc = mal_encoder_encode_octet(encoder, cursor, list_element);
      if (rc < 0)
        return rc;
    }
  }
...
int mal_octet_list_decode_malbinary(mal_octet_list_t *self,
  ...
  self->content = (mal_octet_t *) calloc(self->element_count, sizeof(mal_octet_t *));
  ...
  for (int i = 0; i < self->element_count; i++)
  {
    ...
    {
      rc = mal_decoder_decode_octet(decoder, cursor, &self->content[i]);
      if (rc < 0)
        return rc;
    }
  }
