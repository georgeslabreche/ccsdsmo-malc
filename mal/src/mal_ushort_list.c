...
mal_ushort_list_t *mal_ushort_list_new(unsigned int element_count) {
  mal_ushort_list_t *self = (mal_ushort_list_t *) malloc(
      sizeof(mal_ushort_list_t));
  if (!self)
    return NULL;
  ...
  self->content = (mal_ushort_t *) malloc(
      sizeof(mal_ushort_t) * element_count);
...
int mal_ushort_list_add_encoding_length_malbinary(mal_ushort_list_t *self,
  ...
  mal_ushort_t *content = self->content;
  // Presence flags
  malbinary_add_length((malbinary_cursor_t *) cursor, list_size);
  for (int i = 0; i < list_size; i++) {
    if (presence_flags[i]) {
      mal_ushort_t list_element = content[i];
      rc = malbinary_encoder_add_ushort_encoding_length(encoder, list_element, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int mal_ushort_list_encode_malbinary(mal_ushort_list_t *self,
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (present) {
      mal_ushort_t list_element = content[i];
      rc = malbinary_encoder_encode_ushort(encoder, cursor,
          list_element);
      if (rc < 0)
        return rc;
    }
...
int mal_ushort_list_decode_malbinary(mal_ushort_list_t *self,
  ...
  mal_ushort_t *list_content = (mal_ushort_t *) malloc(
      sizeof(mal_ushort_t) * list_size);
  for (int i = 0; i < list_size; i++) {
    mal_ushort_t list_element;
    bool presence_flag;
    ...
    if (presence_flag) {
      rc = malbinary_decoder_decode_ushort(decoder, cursor,
          &list_element);
      if (rc < 0)
        return rc;
