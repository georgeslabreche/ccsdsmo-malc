...
mal_ulong_list_t *mal_ulong_list_new(unsigned int element_count) {
  mal_ulong_list_t *self = (mal_ulong_list_t *) malloc(
      sizeof(mal_ulong_list_t));
  if (!self)
    return NULL;
  ...
  self->content = (mal_ulong_t *) malloc(
      sizeof(mal_ulong_t) * element_count);
...
int mal_ulong_list_add_encoding_length_malbinary(mal_ulong_list_t *self,
  ...
  mal_ulong_t *content = self->content;
  // Presence flags
  malbinary_add_length((malbinary_cursor_t *) cursor, list_size);
  for (int i = 0; i < list_size; i++) {
    if (presence_flags[i]) {
      mal_ulong_t list_element = content[i];
      rc = malbinary_encoder_add_ulong_encoding_length(encoder, list_element, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int mal_ulong_list_encode_malbinary(mal_ulong_list_t *self,
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (present) {
      mal_ulong_t list_element = content[i];
      rc = malbinary_encoder_encode_ulong(encoder, cursor,
          list_element);
      if (rc < 0)
        return rc;
    }
...
int mal_ulong_list_decode_malbinary(mal_ulong_list_t *self,
  ...
  mal_ulong_t *list_content = (mal_ulong_t *) malloc(
      sizeof(mal_ulong_t) * list_size);
  for (int i = 0; i < list_size; i++) {
    mal_ulong_t list_element;
    bool presence_flag;
    ...
    if (presence_flag) {
      rc = malbinary_decoder_decode_ulong(decoder, cursor,
          &list_element);
      if (rc < 0)
        return rc;
