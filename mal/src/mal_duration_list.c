...
struct _mal_duration_list_t {
  unsigned int element_count;
  bool *presence_flags;
  mal_duration_t *content;
};

mal_duration_list_t *mal_duration_list_new(unsigned int element_count) {
  mal_duration_list_t *self = (mal_duration_list_t *) malloc(
      sizeof(mal_duration_list_t));
  if (!self)
    return NULL;
  ...
  self->content = (mal_duration_t *) malloc(
      sizeof(mal_duration_t) * element_count);
...
void mal_duration_list_destroy(mal_duration_list_t **self_p) {
  if (*self_p) {
    mal_duration_list_t *self = *self_p;
...
int mal_duration_list_add_encoding_length_malbinary(mal_duration_list_t *self,
  ...
  mal_duration_t *content = self->content;
  // Presence flags
  malbinary_add_length((malbinary_cursor_t *) cursor, list_size);
  for (int i = 0; i < list_size; i++) {
    if (presence_flags[i]) {
      mal_duration_t list_element = content[i];
      rc = malbinary_encoder_add_duration_encoding_length(encoder, list_element, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int mal_duration_list_encode_malbinary(mal_duration_list_t *self,
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (present) {
      mal_duration_t list_element = content[i];
      rc = malbinary_encoder_encode_duration(encoder, cursor,
          list_element);
      if (rc < 0)
        return rc;
    }
...
int mal_duration_list_decode_malbinary(mal_duration_list_t *self,
  ...
  mal_duration_t *list_content = (mal_duration_t *) malloc(
      sizeof(mal_duration_t) * list_size);
  for (int i = 0; i < list_size; i++) {
    mal_duration_t list_element;
    bool presence_flag;
    ...
    if (presence_flag) {
      rc = malbinary_decoder_decode_duration(decoder, cursor,
          &list_element);
      if (rc < 0)
        return rc;
