#include "mal.h"

struct _mal_updateheader_list_t {
  unsigned int element_count;
  mal_updateheader_t ** content;
};

// default constructor
mal_updateheader_list_t * mal_updateheader_list_new(unsigned int element_count)
{
  mal_updateheader_list_t *self = (mal_updateheader_list_t *) calloc(1, sizeof(mal_updateheader_list_t));
  if (!self)
    return NULL;
  self->element_count = element_count;
  self->content = (mal_updateheader_t **) calloc(element_count, sizeof(mal_updateheader_t *));
  if (!self->content && (element_count > 0))
  {
    free(self);
    return NULL;
  }
  return self;
}

// destructor, free the list, its content and its elements
void mal_updateheader_list_destroy(mal_updateheader_list_t ** self_p)
{
  if ((*self_p)->element_count > 0)
  {
    for (int i = 0; i < (*self_p)->element_count; i++)
    {
      if ((*self_p)->content[i] != NULL)
        mal_updateheader_destroy(&(*self_p)->content[i]);
    }
    free((*self_p)->content);
  }
  free (*self_p);
  (*self_p) = NULL;
}

// fields accessors for composite list mal_updateheader_list
unsigned int mal_updateheader_list_get_element_count(mal_updateheader_list_t * self)
{
  return self->element_count;
}
mal_updateheader_t ** mal_updateheader_list_get_content(mal_updateheader_list_t * self)
{
  return self->content;
}

// encoding functions related to transport malbinary
int mal_updateheader_list_add_encoding_length_malbinary(mal_updateheader_list_t * self, mal_encoder_t * mal_encoder, void * cursor)
{
  int rc = 0;
  unsigned int list_size = self->element_count;
  mal_encoder_add_list_size_encoding_length(mal_encoder, list_size, cursor);
  for (int i = 0; i < list_size; i++)
  {
    mal_updateheader_t * list_element = self->content[i];
    bool presence_flag = (list_element != NULL);
    mal_encoder_add_presence_flag_encoding_length(mal_encoder, cursor, presence_flag);
    if (presence_flag)
    {
      rc = mal_updateheader_add_encoding_length_malbinary(list_element, mal_encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
  return rc;
}
int mal_updateheader_list_encode_malbinary(mal_updateheader_list_t * self, mal_encoder_t * mal_encoder, void * cursor)
{
  int rc = 0;
  unsigned int list_size = self->element_count;
  rc = mal_encoder_encode_list_size(mal_encoder, cursor, list_size);
  if (rc < 0)
    return rc;
  mal_updateheader_t ** content = self->content;
  for (int i = 0; i < list_size; i++)
  {
    mal_updateheader_t *list_element = content[i];
    bool presence_flag = (list_element != NULL);
    rc = mal_encoder_encode_presence_flag(mal_encoder, cursor, presence_flag);
    if (rc < 0)
      return rc;
    if (presence_flag)
    {
      rc = mal_updateheader_encode_malbinary(list_element, mal_encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
  return rc;
}
int mal_updateheader_list_decode_malbinary(mal_updateheader_list_t * self, mal_decoder_t * mal_decoder, void * cursor)
{
  int rc = mal_decoder_decode_list_size(mal_decoder, cursor, &self->element_count);
  if (rc < 0)
    return rc;
  if (self->element_count == 0)
  {
    self->content = NULL;
    return 0;
  }
  self->content = (mal_updateheader_t **) calloc(self->element_count, sizeof(mal_updateheader_t *));
  if (self->content == NULL)
    return -1;
  for (int i = 0; i < self->element_count; i++)
  {
    bool presence_flag;
    rc = mal_decoder_decode_presence_flag(mal_decoder, cursor, &presence_flag);
    if (rc < 0)
      return rc;
    if (presence_flag)
    {
      self->content[i] = mal_updateheader_new();
      if (self->content[i] == NULL) return -1;
      rc = mal_updateheader_decode_malbinary(self->content[i], mal_decoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
  return rc;
}

// test function
void mal_updateheader_list_test(bool verbose)
{
  printf(" * list of mal_updateheader: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
