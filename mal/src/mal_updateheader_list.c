...
int mal_updateheader_list_add_encoding_length_malbinary(
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (list_element != NULL) {
      rc = mal_updateheader_add_encoding_length_malbinary(list_element, encoder,
          encoding_length);
      if (rc < 0)
        return rc;
    }
...
int mal_updateheader_list_encode_malbinary(mal_updateheader_list_t *self,
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (rc < 0)
      return rc;
    if (presence_flag) {
      rc = mal_updateheader_encode_malbinary(list_element, encoder, bytes,
          offset);
      if (rc < 0)
        return rc;
    }
...
int mal_updateheader_list_decode_malbinary(mal_updateheader_list_t *self,
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (presence_flag) {
      ...
      rc = mal_updateheader_decode_malbinary(list_element, decoder, bytes,
          offset);
      if (rc < 0)
        return rc;
    } else {
