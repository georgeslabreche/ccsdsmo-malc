...
{
  mal_interactiontype_list_t *self = (mal_interactiontype_list_t *) calloc(1, sizeof(mal_interactiontype_list_t));
  if (!self)
    return NULL;
...
int mal_interactiontype_list_add_encoding_length_malbinary(mal_interactiontype_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  int rc = 0;
...
int mal_interactiontype_list_encode_malbinary(mal_interactiontype_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  int rc = 0;
  unsigned int list_size = self->element_count;
...
int mal_interactiontype_list_decode_malbinary(mal_interactiontype_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  int rc = 0;
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      ...
      if (rc < 0)
        return rc;
      self->content[i] = (mal_interactiontype_t) enumerated_value;
    }
  }
  return rc;
}
...
{
  printf(" * list of mal_interactiontype: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
