...
int mal_pair_add_encoding_length_malbinary(mal_pair_t * self, malbinary_encoder_t * malbinary_encoder, void * cursor)
{
  ...
  if (self->first_is_present)
  {
    ...
    rc = malbinary_encoder_add_attribute_encoding_length(malbinary_encoder, self->first_attribute_tag, self->first, cursor);
    if (rc < 0)
      return rc;
  }
  ((malbinary_cursor_t *) cursor)->body_length += MALBINARY_PRESENCE_FLAG_SIZE;
  ...
  {
    ((malbinary_cursor_t *) cursor)->body_length += MALBINARY_ATTRIBUTE_TAG_SIZE;
    rc = malbinary_encoder_add_attribute_encoding_length(malbinary_encoder, self->second_attribute_tag, self->second, cursor);
    if (rc < 0)
      return rc;
  }
  return rc;
...
int mal_pair_encode_malbinary(mal_pair_t * self, malbinary_encoder_t * malbinary_encoder, void * cursor)
{
  ...
  if (presence_flag)
  {
    rc = malbinary_encoder_encode_attribute_tag(malbinary_encoder, cursor, self->first_attribute_tag);
    if (rc < 0)
      return rc;
    rc = malbinary_encoder_encode_attribute(malbinary_encoder, cursor, self->first_attribute_tag, self->first);
    if (rc < 0)
      return rc;
  }
  ...
  if (presence_flag)
  {
    rc = malbinary_encoder_encode_attribute_tag(malbinary_encoder, cursor, self->second_attribute_tag);
    if (rc < 0)
      return rc;
    rc = malbinary_encoder_encode_attribute(malbinary_encoder, cursor, self->second_attribute_tag, self->second);
    if (rc < 0)
      return rc;
  }
  return rc;
...
int mal_pair_decode_malbinary(mal_pair_t * self, malbinary_decoder_t * malbinary_decoder, void * cursor)
{
  ...
  if (presence_flag)
  {
    rc = malbinary_decoder_decode_attribute_tag(malbinary_decoder, cursor, &self->first_attribute_tag);
    if (rc < 0)
      return rc;
    rc = malbinary_decoder_decode_attribute(malbinary_decoder, cursor, self->first_attribute_tag, self->first);
    if (rc < 0)
      return rc;
  }
  ...
  if (presence_flag)
  {
    rc = malbinary_decoder_decode_attribute_tag(malbinary_decoder, cursor, &self->second_attribute_tag);
    if (rc < 0)
      return rc;
    rc = malbinary_decoder_decode_attribute(malbinary_decoder, cursor, self->second_attribute_tag, self->second);
    if (rc < 0)
      return rc;
  }
