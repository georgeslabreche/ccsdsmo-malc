...
int mal_blob_list_add_encoding_length_malbinary(mal_blob_list_t *self,
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (list_element != NULL) {
      rc = malbinary_encoder_add_blob_encoding_length(encoder, list_element, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int mal_blob_list_encode_malbinary(mal_blob_list_t *self,
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (rc < 0)
      return rc;
    if (presence_flag) {
      rc = malbinary_encoder_encode_blob(encoder, cursor,
          list_element);
      if (rc < 0)
        return rc;
    }
...
int mal_blob_list_decode_malbinary(mal_blob_list_t *self,
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (rc < 0)
      return rc;
    if (presence_flag) {
      rc = malbinary_decoder_decode_blob(decoder, cursor,
          &list_element);
      if (rc < 0)
        return rc;
    } else {
