...
mal_blob_list_t *mal_blob_list_new(unsigned int element_count) {
  mal_blob_list_t *self = (mal_blob_list_t *) malloc(sizeof(mal_blob_list_t));
  if (!self)
    return NULL;

  self->element_count = element_count;
  self->content = (mal_blob_t **) malloc(sizeof(mal_blob_t *) * element_count);
...
int mal_blob_list_add_encoding_length_malbinary(mal_blob_list_t *self,
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (list_element != NULL) {
      rc = malbinary_encoder_add_blob_encoding_length(encoder, list_element,
          encoding_length);
      if (rc < 0)
        return rc;
    }
...
int mal_blob_list_encode_malbinary(mal_blob_list_t *self,
  ...
  for (int i = 0; i < list_size; i++) {
    ...
    if (rc < 0)
      return rc;
    if (presence_flag) {
      rc = malbinary_encoder_encode_blob(encoder, bytes, offset,
          list_element);
      if (rc < 0)
        return rc;
    }
...
int mal_blob_list_decode_malbinary(mal_blob_list_t *self,
  ...
  mal_blob_t **list_content = (mal_blob_t **) malloc(
      sizeof(mal_blob_t *) * list_size);
  for (int i = 0; i < list_size; i++) {
    mal_blob_t *list_element;
    bool presence_flag;
    ...
    if (rc < 0)
      return rc;
    if (presence_flag) {
      rc = malbinary_decoder_decode_blob(decoder, bytes, offset,
          &list_element);
      if (rc < 0)
        return rc;
    } else {
