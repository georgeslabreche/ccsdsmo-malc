/*  =========================================================================
    sepp_tm_selftest.c - run selftests
...
{
    test_item_t *item;
    printf ("Running sepp_tm selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef SEPP_TM_BUILD_DRAFT_API // selftest is still in draft
        else
            sepp_tm_private_selftest (verbose, item->subtest);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("sepp_tm_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running sepp_tm test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef SEPP_TM_BUILD_DRAFT_API // selftest is still in draft
        else
            sepp_tm_private_selftest (verbose, test->subtest);
#endif // SEPP_TM_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
