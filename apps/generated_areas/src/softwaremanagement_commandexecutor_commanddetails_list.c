...
struct _softwaremanagement_commandexecutor_commanddetails_list_t {
  unsigned int element_count;
  softwaremanagement_commandexecutor_commanddetails_t ** content;
};
...
{
  softwaremanagement_commandexecutor_commanddetails_list_t *self = (softwaremanagement_commandexecutor_commanddetails_list_t *) calloc(1, sizeof(softwaremanagement_commandexecutor_commanddetails_list_t));
  if (!self)
    return NULL;
  self->element_count = element_count;
  self->content = (softwaremanagement_commandexecutor_commanddetails_t **) calloc(element_count, sizeof(softwaremanagement_commandexecutor_commanddetails_t *));
  ...
  if (self_p && *self_p)
  {
    for (int i = 0; i < (*self_p)->element_count; i++)
    {
      if ((*self_p)->content[i] != NULL)
        softwaremanagement_commandexecutor_commanddetails_destroy(&(*self_p)->content[i]);
    }
    free((*self_p)->content);
  }
  free (*self_p);
...
int softwaremanagement_commandexecutor_commanddetails_list_add_encoding_length_malbinary(softwaremanagement_commandexecutor_commanddetails_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  {
    softwaremanagement_commandexecutor_commanddetails_t * list_element = self->content[i];
    ...
    if (presence_flag)
    {
      rc = softwaremanagement_commandexecutor_commanddetails_add_encoding_length_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int softwaremanagement_commandexecutor_commanddetails_list_encode_malbinary(softwaremanagement_commandexecutor_commanddetails_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  if (rc < 0)
    return rc;
  softwaremanagement_commandexecutor_commanddetails_t ** content = self->content;
  for (int i = 0; i < list_size; i++)
  {
    softwaremanagement_commandexecutor_commanddetails_t *list_element = content[i];
    bool presence_flag = (list_element != NULL);
    ...
    if (presence_flag)
    {
      rc = softwaremanagement_commandexecutor_commanddetails_encode_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int softwaremanagement_commandexecutor_commanddetails_list_decode_malbinary(softwaremanagement_commandexecutor_commanddetails_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  self->content = (softwaremanagement_commandexecutor_commanddetails_t **) calloc(list_size, sizeof(softwaremanagement_commandexecutor_commanddetails_t *));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      self->content[i] = softwaremanagement_commandexecutor_commanddetails_new();
      if (self->content[i] == NULL) return -1;
      rc = softwaremanagement_commandexecutor_commanddetails_decode_malbinary(self->content[i], decoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
{
  printf(" * list of softwaremanagement_commandexecutor_commanddetails: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
