...
struct _pf_camera_pictureformat_list_t {
  unsigned int element_count;
  bool * presence_flags;
  pf_camera_pictureformat_t * content;
};

// default constructor
pf_camera_pictureformat_list_t * pf_camera_pictureformat_list_new(unsigned int element_count)
{
  pf_camera_pictureformat_list_t *self = (pf_camera_pictureformat_list_t *) calloc(1, sizeof(pf_camera_pictureformat_list_t));
  if (!self)
    return NULL;
  ...
  self->content = (pf_camera_pictureformat_t *) calloc(element_count, sizeof(pf_camera_pictureformat_t));
  if (!self->content)
  {
...
int pf_camera_pictureformat_list_decode_malbinary(pf_camera_pictureformat_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  if (self->presence_flags == NULL)
    return -1;
  self->content = (pf_camera_pictureformat_t *) calloc(list_size, sizeof(pf_camera_pictureformat_t));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      ...
      if (rc < 0)
        return rc;
      self->content[i] = (pf_camera_pictureformat_t) enumerated_value;
    }
  }
  return rc;
}
...
{
  printf(" * list of pf_camera_pictureformat: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
