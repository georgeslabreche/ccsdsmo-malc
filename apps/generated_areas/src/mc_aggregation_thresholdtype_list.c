...
struct _mc_aggregation_thresholdtype_list_t {
  unsigned int element_count;
  bool * presence_flags;
  mc_aggregation_thresholdtype_t * content;
};

// default constructor
mc_aggregation_thresholdtype_list_t * mc_aggregation_thresholdtype_list_new(unsigned int element_count)
{
  mc_aggregation_thresholdtype_list_t *self = (mc_aggregation_thresholdtype_list_t *) calloc(1, sizeof(mc_aggregation_thresholdtype_list_t));
  if (!self)
    return NULL;
  ...
  self->content = (mc_aggregation_thresholdtype_t *) calloc(element_count, sizeof(mc_aggregation_thresholdtype_t));
  if (!self->content)
  {
...
int mc_aggregation_thresholdtype_list_decode_malbinary(mc_aggregation_thresholdtype_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  if (self->presence_flags == NULL)
    return -1;
  self->content = (mc_aggregation_thresholdtype_t *) calloc(list_size, sizeof(mc_aggregation_thresholdtype_t));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      ...
      if (rc < 0)
        return rc;
      self->content[i] = (mc_aggregation_thresholdtype_t) enumerated_value;
    }
  }
  return rc;
}
...
{
  printf(" * list of mc_aggregation_thresholdtype: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
