...
struct _common_servicekey_list_t {
  unsigned int element_count;
  common_servicekey_t ** content;
};

// default constructor
common_servicekey_list_t * common_servicekey_list_new(unsigned int element_count)
{
  common_servicekey_list_t *self = (common_servicekey_list_t *) calloc(1, sizeof(common_servicekey_list_t));
  if (!self)
    return NULL;
  self->element_count = element_count;
  self->content = (common_servicekey_t **) calloc(element_count, sizeof(common_servicekey_t *));
  ...
  if (self_p && *self_p)
  {
    for (int i = 0; i < (*self_p)->element_count; i++)
    {
      if ((*self_p)->content[i] != NULL)
        common_servicekey_destroy(&(*self_p)->content[i]);
    }
    free((*self_p)->content);
  }
  free (*self_p);
...
int common_servicekey_list_add_encoding_length_malbinary(common_servicekey_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  {
    common_servicekey_t * list_element = self->content[i];
    ...
    if (presence_flag)
    {
      rc = common_servicekey_add_encoding_length_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int common_servicekey_list_encode_malbinary(common_servicekey_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  if (rc < 0)
    return rc;
  common_servicekey_t ** content = self->content;
  for (int i = 0; i < list_size; i++)
  {
    common_servicekey_t *list_element = content[i];
    bool presence_flag = (list_element != NULL);
    ...
    if (presence_flag)
    {
      rc = common_servicekey_encode_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int common_servicekey_list_decode_malbinary(common_servicekey_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  self->content = (common_servicekey_t **) calloc(list_size, sizeof(common_servicekey_t *));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      self->content[i] = common_servicekey_new();
      if (self->content[i] == NULL) return -1;
      rc = common_servicekey_decode_malbinary(self->content[i], decoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
{
  printf(" * list of common_servicekey: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
