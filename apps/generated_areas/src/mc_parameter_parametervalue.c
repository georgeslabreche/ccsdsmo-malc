...
mal_uoctet_t mc_parameter_parametervalue_get_validitystate(mc_parameter_parametervalue_t * self)
{
  return self->f_validitystate;
}
void mc_parameter_parametervalue_set_validitystate(mc_parameter_parametervalue_t * self, mal_uoctet_t f_validitystate)
{
...
{
  int rc = 0;
  rc = mal_encoder_add_uoctet_encoding_length(encoder, self->f_validitystate, cursor);
  if (rc < 0)
    return rc;
...
int mc_parameter_parametervalue_encode_malbinary(mc_parameter_parametervalue_t * self, mal_encoder_t * encoder, void * cursor)
{
  int rc = 0;
  bool presence_flag;
  rc = mal_encoder_encode_uoctet(encoder, cursor, self->f_validitystate);
  if (rc < 0)
    return rc;
...
int mc_parameter_parametervalue_decode_malbinary(mc_parameter_parametervalue_t * self, mal_decoder_t * decoder, void * cursor)
{
  int rc = 0;
  bool presence_flag;
  rc = mal_decoder_decode_uoctet(decoder, cursor, &self->f_validitystate);
  if (rc < 0)
    return rc;
