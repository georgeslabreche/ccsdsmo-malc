...
struct _mc_check_checkresultfilter_t {
  mal_boolean_t f_checkfilterviagroups;
  mal_long_list_t * f_checkfilter;
  mal_boolean_t f_parameterfilterviagroups;
...
{
  return self->f_checkfilterviagroups;
}
...
{
  self->f_checkfilterviagroups = f_checkfilterviagroups;
}
...
{
  return self->f_parameterfilterviagroups;
}
...
{
  self->f_parameterfilterviagroups = f_parameterfilterviagroups;
}
...
{
  int rc = 0;
  rc = mal_encoder_add_boolean_encoding_length(encoder, self->f_checkfilterviagroups, cursor);
  if (rc < 0)
    return rc;
  rc = mal_long_list_add_encoding_length_malbinary(self->f_checkfilter, encoder, cursor);
  if (rc < 0)
    return rc;
  rc = mal_encoder_add_boolean_encoding_length(encoder, self->f_parameterfilterviagroups, cursor);
  if (rc < 0)
    return rc;
...
{
  int rc = 0;
  rc = mal_encoder_encode_boolean(encoder, cursor, self->f_checkfilterviagroups);
  if (rc < 0)
    return rc;
  rc = mal_long_list_encode_malbinary(self->f_checkfilter, encoder, cursor);
  if (rc < 0)
    return rc;
  rc = mal_encoder_encode_boolean(encoder, cursor, self->f_parameterfilterviagroups);
  if (rc < 0)
    return rc;
...
{
  int rc = 0;
  rc = mal_decoder_decode_boolean(decoder, cursor, &self->f_checkfilterviagroups);
  if (rc < 0)
    return rc;
  ...
  if (rc < 0)
    return rc;
  rc = mal_decoder_decode_boolean(decoder, cursor, &self->f_parameterfilterviagroups);
  if (rc < 0)
    return rc;
