...
struct _opssat_std_memorymanagement_memorydevice_list_t {
  unsigned int element_count;
  bool * presence_flags;
  opssat_std_memorymanagement_memorydevice_t * content;
};
...
{
  opssat_std_memorymanagement_memorydevice_list_t *self = (opssat_std_memorymanagement_memorydevice_list_t *) calloc(1, sizeof(opssat_std_memorymanagement_memorydevice_list_t));
  if (!self)
    return NULL;
  ...
  self->content = (opssat_std_memorymanagement_memorydevice_t *) calloc(element_count, sizeof(opssat_std_memorymanagement_memorydevice_t));
  if (!self->content)
  {
...
int opssat_std_memorymanagement_memorydevice_list_decode_malbinary(opssat_std_memorymanagement_memorydevice_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  if (self->presence_flags == NULL)
    return -1;
  self->content = (opssat_std_memorymanagement_memorydevice_t *) calloc(list_size, sizeof(opssat_std_memorymanagement_memorydevice_t));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      ...
      if (rc < 0)
        return rc;
      self->content[i] = (opssat_std_memorymanagement_memorydevice_t) enumerated_value;
    }
  }
  return rc;
}
...
{
  printf(" * list of opssat_std_memorymanagement_memorydevice: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
