...
struct _softwaremanagement_packagemanagement_packagedefinition_list_t {
  unsigned int element_count;
  softwaremanagement_packagemanagement_packagedefinition_t ** content;
};
...
{
  softwaremanagement_packagemanagement_packagedefinition_list_t *self = (softwaremanagement_packagemanagement_packagedefinition_list_t *) calloc(1, sizeof(softwaremanagement_packagemanagement_packagedefinition_list_t));
  if (!self)
    return NULL;
  self->element_count = element_count;
  self->content = (softwaremanagement_packagemanagement_packagedefinition_t **) calloc(element_count, sizeof(softwaremanagement_packagemanagement_packagedefinition_t *));
  ...
  if (self_p && *self_p)
  {
    for (int i = 0; i < (*self_p)->element_count; i++)
    {
      if ((*self_p)->content[i] != NULL)
        softwaremanagement_packagemanagement_packagedefinition_destroy(&(*self_p)->content[i]);
    }
    free((*self_p)->content);
  }
  free (*self_p);
...
int softwaremanagement_packagemanagement_packagedefinition_list_add_encoding_length_malbinary(softwaremanagement_packagemanagement_packagedefinition_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  {
    softwaremanagement_packagemanagement_packagedefinition_t * list_element = self->content[i];
    ...
    if (presence_flag)
    {
      rc = softwaremanagement_packagemanagement_packagedefinition_add_encoding_length_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int softwaremanagement_packagemanagement_packagedefinition_list_encode_malbinary(softwaremanagement_packagemanagement_packagedefinition_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  if (rc < 0)
    return rc;
  softwaremanagement_packagemanagement_packagedefinition_t ** content = self->content;
  for (int i = 0; i < list_size; i++)
  {
    softwaremanagement_packagemanagement_packagedefinition_t *list_element = content[i];
    bool presence_flag = (list_element != NULL);
    ...
    if (presence_flag)
    {
      rc = softwaremanagement_packagemanagement_packagedefinition_encode_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int softwaremanagement_packagemanagement_packagedefinition_list_decode_malbinary(softwaremanagement_packagemanagement_packagedefinition_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  self->content = (softwaremanagement_packagemanagement_packagedefinition_t **) calloc(list_size, sizeof(softwaremanagement_packagemanagement_packagedefinition_t *));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      self->content[i] = softwaremanagement_packagemanagement_packagedefinition_new();
      if (self->content[i] == NULL) return -1;
      rc = softwaremanagement_packagemanagement_packagedefinition_decode_malbinary(self->content[i], decoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
{
  printf(" * list of softwaremanagement_packagemanagement_packagedefinition: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
