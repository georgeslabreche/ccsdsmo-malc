...
struct _com_objecttype_list_t {
  unsigned int element_count;
  com_objecttype_t ** content;
};

// default constructor
com_objecttype_list_t * com_objecttype_list_new(unsigned int element_count)
{
  com_objecttype_list_t *self = (com_objecttype_list_t *) calloc(1, sizeof(com_objecttype_list_t));
  if (!self)
    return NULL;
  self->element_count = element_count;
  self->content = (com_objecttype_t **) calloc(element_count, sizeof(com_objecttype_t *));
  ...
  if (self_p && *self_p)
  {
    for (int i = 0; i < (*self_p)->element_count; i++)
    {
      if ((*self_p)->content[i] != NULL)
        com_objecttype_destroy(&(*self_p)->content[i]);
    }
    free((*self_p)->content);
  }
  free (*self_p);
...
int com_objecttype_list_add_encoding_length_malbinary(com_objecttype_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  {
    com_objecttype_t * list_element = self->content[i];
    ...
    if (presence_flag)
    {
      rc = com_objecttype_add_encoding_length_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int com_objecttype_list_encode_malbinary(com_objecttype_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  if (rc < 0)
    return rc;
  com_objecttype_t ** content = self->content;
  for (int i = 0; i < list_size; i++)
  {
    com_objecttype_t *list_element = content[i];
    bool presence_flag = (list_element != NULL);
    ...
    if (presence_flag)
    {
      rc = com_objecttype_encode_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int com_objecttype_list_decode_malbinary(com_objecttype_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  self->content = (com_objecttype_t **) calloc(list_size, sizeof(com_objecttype_t *));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      self->content[i] = com_objecttype_new();
      if (self->content[i] == NULL) return -1;
      rc = com_objecttype_decode_malbinary(self->content[i], decoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
