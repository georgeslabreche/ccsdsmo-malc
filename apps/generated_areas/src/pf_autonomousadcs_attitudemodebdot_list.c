...
struct _pf_autonomousadcs_attitudemodebdot_list_t {
  unsigned int element_count;
  pf_autonomousadcs_attitudemodebdot_t ** content;
};

// default constructor
pf_autonomousadcs_attitudemodebdot_list_t * pf_autonomousadcs_attitudemodebdot_list_new(unsigned int element_count)
{
  pf_autonomousadcs_attitudemodebdot_list_t *self = (pf_autonomousadcs_attitudemodebdot_list_t *) calloc(1, sizeof(pf_autonomousadcs_attitudemodebdot_list_t));
  if (!self)
    return NULL;
  self->element_count = element_count;
  self->content = (pf_autonomousadcs_attitudemodebdot_t **) calloc(element_count, sizeof(pf_autonomousadcs_attitudemodebdot_t *));
  ...
  if (self_p && *self_p)
  {
    for (int i = 0; i < (*self_p)->element_count; i++)
    {
      if ((*self_p)->content[i] != NULL)
        pf_autonomousadcs_attitudemodebdot_destroy(&(*self_p)->content[i]);
    }
    free((*self_p)->content);
  }
  free (*self_p);
...
int pf_autonomousadcs_attitudemodebdot_list_add_encoding_length_malbinary(pf_autonomousadcs_attitudemodebdot_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  {
    pf_autonomousadcs_attitudemodebdot_t * list_element = self->content[i];
    ...
    if (presence_flag)
    {
      rc = pf_autonomousadcs_attitudemodebdot_add_encoding_length_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int pf_autonomousadcs_attitudemodebdot_list_encode_malbinary(pf_autonomousadcs_attitudemodebdot_list_t * self, mal_encoder_t * encoder, void * cursor)
{
  ...
  if (rc < 0)
    return rc;
  pf_autonomousadcs_attitudemodebdot_t ** content = self->content;
  for (int i = 0; i < list_size; i++)
  {
    pf_autonomousadcs_attitudemodebdot_t *list_element = content[i];
    bool presence_flag = (list_element != NULL);
    ...
    if (presence_flag)
    {
      rc = pf_autonomousadcs_attitudemodebdot_encode_malbinary(list_element, encoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
int pf_autonomousadcs_attitudemodebdot_list_decode_malbinary(pf_autonomousadcs_attitudemodebdot_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  self->content = (pf_autonomousadcs_attitudemodebdot_t **) calloc(list_size, sizeof(pf_autonomousadcs_attitudemodebdot_t *));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      self->content[i] = pf_autonomousadcs_attitudemodebdot_new();
      if (self->content[i] == NULL) return -1;
      rc = pf_autonomousadcs_attitudemodebdot_decode_malbinary(self->content[i], decoder, cursor);
      if (rc < 0)
        return rc;
    }
  }
...
{
  printf(" * list of pf_autonomousadcs_attitudemodebdot: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
