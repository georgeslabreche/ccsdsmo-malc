...
struct _mc_parameter_validitystate_list_t {
  unsigned int element_count;
  bool * presence_flags;
  mc_parameter_validitystate_t * content;
};

// default constructor
mc_parameter_validitystate_list_t * mc_parameter_validitystate_list_new(unsigned int element_count)
{
  mc_parameter_validitystate_list_t *self = (mc_parameter_validitystate_list_t *) calloc(1, sizeof(mc_parameter_validitystate_list_t));
  if (!self)
    return NULL;
  ...
  self->content = (mc_parameter_validitystate_t *) calloc(element_count, sizeof(mc_parameter_validitystate_t));
  if (!self->content)
  {
...
int mc_parameter_validitystate_list_decode_malbinary(mc_parameter_validitystate_list_t * self, mal_decoder_t * decoder, void * cursor)
{
  ...
  if (self->presence_flags == NULL)
    return -1;
  self->content = (mc_parameter_validitystate_t *) calloc(list_size, sizeof(mc_parameter_validitystate_t));
  ...
  for (int i = 0; i < list_size; i++)
  {
    ...
    if (presence_flag)
    {
      ...
      if (rc < 0)
        return rc;
      self->content[i] = (mc_parameter_validitystate_t) enumerated_value;
    }
  }
  return rc;
}
...
{
  printf(" * list of mc_parameter_validitystate: ");
  if (verbose)
    printf("\n");
  printf("OK\n");
}
