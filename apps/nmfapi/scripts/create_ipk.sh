...
PKG_NAME=$(sed -n -e '/^Package/p' CONTROL/control | cut -d ' ' -f2)
PKG_VER=$(sed -n -e '/^Version/p' CONTROL/control | cut -d ' ' -f2)
PKG_ARCH=$(sed -n -e '/^Architecture/p' CONTROL/control | cut -d ' ' -f2)
