/*  =========================================================================
    nmfapi_c_selftest.c - run selftests
...
{
    test_item_t *item;
    printf ("Running nmfapi_c selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef NMFAPI_C_BUILD_DRAFT_API // selftest is still in draft
        else
            nmfapi_c_private_selftest (verbose, item->subtest);
#endif // NMFAPI_C_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("nmfapi_c_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running nmfapi_c test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef NMFAPI_C_BUILD_DRAFT_API // selftest is still in draft
        else
            nmfapi_c_private_selftest (verbose, test->subtest);
#endif // NMFAPI_C_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
